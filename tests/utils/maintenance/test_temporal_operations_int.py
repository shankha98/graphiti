...
async def test_get_edge_contradictions():
    ...
    invalidated_edges = await get_edge_contradictions(
        setup_llm_client(), new_edge, [existing_edge]
    )
...
def create_complex_test_data():
    ...
    # Create nodes
    node1 = EntityNode(
        uuid="1", name="Alice", labels=["Person"], created_at=now, group_id="1"
    )
    node2 = EntityNode(
        uuid="2", name="Bob", labels=["Person"], created_at=now, group_id="1"
    )
    node3 = EntityNode(
        uuid="3", name="Charlie", labels=["Person"], created_at=now, group_id="1"
    )
    node4 = EntityNode(
        uuid="4",
        name="Company XYZ",
...
async def test_invalidate_edges_complex():
    ...
    invalidated_edges = await get_edge_contradictions(
        setup_llm_client(), new_edge, existing_edges
    )
